        ...
        <div className="app-shell">
          ...
          <main className="page-container dashboard">
            <section className="search-card">
              ...
              <form className="search-controls" onSubmit={event => {
                event.preventDefault();
                fetchTransactions();
              }}>
                <input id="datetime" type="date" className="app-input date-input" value={dateTime} onChange={handleDateTimeChange} onClick={event => {
            ...
            <div className="overflow-x-auto"><table className="data-table">
                ...
                <tbody>
                    {transactions.map((t) => (
                        <tr key={t.id}>
                            <td>{moment.tz(t.created_at, "YYYY-MM-DD HH:mm:ss", "Asia/Bangkok").format("YYYY-MM-DD HH:mm:ss")}</td>
                            <td>{t.description}</td>
                            <td className={Number(t.amount) >= 0 ? 'amount-income' : 'amount-expense'}>{Number(t.amount).toLocaleString('th-TH', { minimumFractionDigits: 2 })}</td>
            ...
            {transactions.length === 0 && dateTime && <div className="empty-state"><strong>ไม่พบรายการ</strong>ลองเลือกวันอื่น หรือเพิ่มรายการใหม่จากหน้าหลัก</div>}
